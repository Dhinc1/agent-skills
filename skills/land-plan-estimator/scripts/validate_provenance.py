#!/usr/bin/env python3
"""validate_provenance.py — the accuracy fix. For every DB row with a
quotable ATOMIC value and a claimed source sheet, confirm the value's
distinctive tokens actually appear in that sheet's text layer. If the cited
sheet lacks them (wrong sheet cited, or the value belongs to an adjacent
sheet), relocate the row to the sheet that actually contains it — or FLAG it
for manual review. This is what catches a value attributed to the wrong
sheet before it ships in a takeoff.

Guards (hardened after real civil stress tests):
  - DESPACING: text and tokens are despaced so letter-spaced CAD text
    ("1 A - 1 1") still validates against "1A-11".
  - PROSE GUARD: skips long/paraphrased value fields (general notes) — only
    validates atomic values (pipe sizes, structure marks, model numbers,
    rim/invert elevations). Tokenising a sentence causes false fails.
  - SAME-DISCIPLINE GATE (--discipline-map): never relocates a row across
    disciplines (won't move a storm structure onto a landscape sheet just
    because a token happens to match).
  - HIGHER BAR: relocates only if target coverage >=0.7 AND (cited sheet
    empty OR target coverage >=3x cited coverage), or the value's
    high-entropy token appears on exactly one sheet in the whole set.

Ported unchanged from ContractorOS's drawings-analyser (pure text-matching,
no unit-system dependency).

Config: structured.json, a directory of per-sheet .txt files (stem matches
the source-sheet value used in the DB, e.g. "sheet_015" -> sheet_015.txt),
and a fields map {table: [id_field, value_field, source_field]}.

Usage:
    python validate_provenance.py <structured.json> --textdir <sheets_dir> \\
        --fields <fields.json> [--discipline-map <map.json>] [--apply] -o <out.json>
"""
import json
import re
import os
...
def tokens(val):
    """Distinctive atomic tokens. Returns a set; high-entropy singletons allowed."""
    up = str(val).upper()
